...
class LoRALinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Cast LoRA matrices to match input dtype (e.g. bfloat16 decoder)
        A = self.A.to(x.dtype)
        B = self.B.to(x.dtype)
...
def apply_lora_to_attention(model: nn.Module, rank: int, alpha: float) -> nn.Module:
    ...
    Freezes ALL model parameters first, then replaces q_proj/v_proj with
    LoRALinear adapters (whose A and B are trainable).
    ...
    # Replace q_proj and v_proj in every attention Head with LoRALinear
